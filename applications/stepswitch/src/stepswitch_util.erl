...
correct_shortdial(Number, OffnetReq) ->
    ...
    CallRestrictionsEnabled = kapi_offnet_resource:call_restrictions_enabled(OffnetReq),
    correct_shortdial(Number, CIDNum, CallRestrictionsEnabled).

-spec correct_shortdial(kz_term:ne_binary(), kz_term:ne_binary(), kz_json:object()) -> kz_term:api_binary().
correct_shortdial(<<"+", Number/binary>>, CIDNum, CallRestrictions) ->
    correct_shortdial(Number, CIDNum, CallRestrictions);
correct_shortdial(Number, <<"+", CIDNum/binary>>, CallRestrictions) ->
    correct_shortdial(Number, CIDNum, CallRestrictions);
correct_shortdial(Number, CIDNum, CallRestrictions) when is_binary(CIDNum) ->
    ...
    case byte_size(CIDNum) - byte_size(Number) of
        Length when Length =< MaxCorrection, Length >= MinCorrection ->
            ...
            'undefined' = kz_json:get_value([Classification, <<"action">>], CallRestrictions),
            CorrectedNumber;
        _ ->
